...
const server = http.createServer(async (req, res) => {
  ...
  if ((pathname === '/api/db/query' || pathname === '/api/data') && (req.method === 'POST' || req.method === 'GET')) {
    try {
      ...
      if (req.method === 'POST') {
        ...
        if (body.driver || body.DriverName) driver = body.driver || body.DriverName;
        if (body.database || body.Database) database = body.database || body.Database;
        if (body.ip || body.IP || body.server || body.Server) host = body.ip || body.IP || body.server || body.Server;
        if (body.porta || body.Porta || body.port || body.Port) port = parseInt(body.porta || body.Porta || body.port || body.Port, 10);
        if (body.login || body.Login || body.user || body.userName) user = body.login || body.Login || body.user || body.userName;
        if (body.senha || body.Senha || body.password) password = body.senha || body.Senha || body.password;
      }

      // Suporte nativo a Firebird (.fdb)
      if (driver === 'Firebird' || (database && database.toLowerCase().endsWith('.fdb'))) {
        try {
          const firebird = require('node-firebird');
          let fullDbPath = database || 'projetos/Relatorio_Clientes_Firebird/dados/clientes.fdb';
          if (!path.isAbsolute(fullDbPath)) {
            const candidate1 = path.resolve(WORKSPACE_DIR, fullDbPath);
            const candidate2 = path.resolve(WORKSPACE_DIR, 'projetos/Relatorio_Clientes_Firebird/dados', path.basename(fullDbPath));
            fullDbPath = fs.existsSync(candidate1) ? candidate1 : (fs.existsSync(candidate2) ? candidate2 : candidate1);
          }
          ...
          const fbRows = await new Promise((resolve, reject) => {
          ...
          return sendJson(res, 200, { rows: fbRows || [], fallback: false, driver: 'Firebird' });
        } catch (fbErr) {
          console.warn('[Server] Falha na consulta Firebird, tentando fallback:', fbErr.message);
        }
      }
  ...
  if (pathname === '/api/components/install-package' && req.method === 'POST') {
    try {
      ...
      const packagesDir = path.resolve(WORKSPACE_DIR, 'packages');
  ...
  if (pathname === '/api/webview/proxy' && req.method === 'GET') {
    ...
    try {
      ...
      // Função auxiliar para seguir redirecionamentos HTTP (ex: 301, 302)
      const fetchWithRedirects = async (initialUrl, maxRedirects = 5) => {
        ...
        while (redirectCount < maxRedirects) {
          ...
          // Se for redirecionamento, segue o Location
          if ([301, 302, 303, 307, 308].includes(response.statusCode) && response.headers.location) {
            ...
            currentUrl = loc;
            redirectCount++;
            response.resume(); // drena stream para liberar socket
            continue;
          }
          ...
          return { res: response, body, finalUrl: currentUrl };
        }
        throw new Error('Muitos redirecionamentos');
      };

      const { res: proxyRes, body: rawBody, finalUrl } = await fetchWithRedirects(targetUrl);
      ...
      const baseTag = `<base href="${parsedFinal.origin}${parsedFinal.pathname}">`;
      const injectScript = `
<script>
...
window.addEventListener('message', function(e) {
  if (e.data && e.data.to === 'webview') {
    if (typeof window.onHostMessage === 'function') window.onHostMessage(e.data.data);
      ...
      res.end(`<html><body style="font-family:sans-serif;padding:24px;background:#f8fafc;color:#334155;">
        <h3 style="color:#ef4444;">🚫 Erro ao carregar URL</h3>
        <p><b>${err.message}</b></p>
        <p>URL solicitada: <code style="color:#0284c7">${targetUrl}</code></p>
      </body></html>`);
      return;
    }
  }
