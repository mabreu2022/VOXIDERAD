...
class VoxReportEngine {
  ...
  async resolveRecords(reportComp, formState, providedRecords = null) {
    ...
    // 2. Tentar buscar do backend SQLite ou Firebird se houver conexão ou query no formulário
    ...
    if (formState && formState.components) {
      ...
      const connComp = formState.components.find(c =>
        (c.type === 'vox_Connection' || c.type === 'TFDConnection') && c.props
      );
      if (connComp) {
        connInfo = {
    ...
    try {
      const res = await fetch('/api/db/query', {
        ...
        body: JSON.stringify({ sql: sqlToRun, ...connInfo })
      });
      ...
      console.warn('[VoxReportEngine] Não foi possível consultar banco diretamente:', e);
    }
        ...
        if (c.type === 'vox_ReportLabel' || c.type === 'TVoxReportLabel' || c.type === 'TQRLabel') {
          ...
          if (record && field) {
            if (record[field] !== undefined) {
              val = record[field];
            } else if (record[field.toUpperCase()] !== undefined) {
              val = record[field.toUpperCase()];
            } else if (record[field.toLowerCase()] !== undefined) {
              val = record[field.toLowerCase()];
            } else {
              const matchedKey = Object.keys(record).find(k => k.toLowerCase() === field.toLowerCase());
    ...
    if (summaryBand) {
      ...
      curPage.bodyHtml += `
        <div class="vox-band vox-band-summary" style="position:relative; width:100%; height:${summaryH}px; background:${summaryBand.props.Color || '#f1f5f9'}; border-top: 2px solid #334155; margin-top: 4px;">
          ${renderBandElements(summaryBand, { saldo: sumValue, SALDO: sumValue, valor: sumValue, VALOR: sumValue, total: sumValue, TOTAL: sumValue, count: records.length, COUNT: records.length }, curPage.pageNumber, 1)}
        </div>
      `;
