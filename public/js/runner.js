...
class VoxFormRunner {
  ...
  computeAlignments(components, width, height) {
    const nonVisual = [
      'vox_DataSource', 'vox_Connection', 'vox_Query', 'vox_DBTable', 'vox_Timer', 'vox_OpenDialog', 'vox_SaveDialog',
      ...
      'TVoxDataSource', 'TVoxConnection', 'TVoxQuery', 'TVoxDBTable', 'TVoxTimer', 'TVoxOpenDialog', 'TVoxSaveDialog',
      ...
      'TDataSource', 'TFDConnection', 'TFDQuery', 'TDBTable', 'TTable', 'TTimer', 'TOpenDialog', 'TSaveDialog'
    ];
    ...
    const formName = this._lastFormName || 'Form1';
    const visual = (components || []).filter(c => !nonVisual.includes(c.type));
    const rootComps = visual.filter(c => !c.parent || c.parent === formName);
  ...
  renderLiveForm(formState) {
    ...
    this._lastFormName = formState.name;
  ...
  renderLiveComponent(comp) {
    ...
    if (comp.type === 'vox_ReportBand' || comp.type === 'TVoxReportBand' || comp.type === 'TQRBand') {
      const bt = comp.props.BandType || 'rbDetail';
      const bg = comp.props.Color || '#ffffff';
      return `
        <div id="live_${comp.id}" class="vcl-report-band" style="width:100%; height:100%; background:${bg}; border-bottom:1px dashed #cbd5e1; position:relative;">
          <div style="position:absolute; top:0; left:0; font-size:8px; font-weight:bold; color:#64748b; padding:1px 4px; background:rgba(0,0,0,0.05); pointer-events:none;">
            ${comp.name} (${bt})
          </div>
        </div>
      `;
    ...
    if (comp.type === 'vox_DBGrid' || comp.type === 'TDBGrid') {
      const formComps = (formState && formState.components) ? formState.components : [];
      ...
      if (!hasValidBinding) {
        ...
        const colHeaders = colList.length > 0 
          ? colList.map(c => `<div>${c}</div>`).join('') 
          : '<div></div><div></div><div></div><div></div>';

        let emptyRowsHtml = '';
        for (let i = 0; i < 6; i++) {
          emptyRowsHtml += `
            <div class="vcl-dbgrid-row">
              <div>&nbsp;</div><div>&nbsp;</div><div>&nbsp;</div><div>&nbsp;</div>
            </div>
          `;
        }
        ...
        return `
          <div class="vcl-dbgrid">
            <div class="vcl-dbgrid-hdr">${colHeaders}</div>
            <div style="flex:1; overflow-y:auto;">${emptyRowsHtml}</div>
          </div>
        `;
      }
      ...
      if (this.activeRecords && this.activeRecords.length > 0) {
        this.activeRecords.slice(0, 15).forEach((rec, idx) => {
          ...
          cols.forEach(colName => {
            ...
            let val = rec[lower] !== undefined ? rec[lower] : rec[colName];
            if (val === undefined) {
              const k = Object.keys(rec).find(x => x.toLowerCase() === lower);
              val = k ? rec[k] : '';
            }
            if ((lower === 'saldo' || lower === 'valor' || lower === 'preco') && val !== '' && !isNaN(val)) {
              val = (parseFloat(val) || 0).toFixed(2);
            }
            cells += `<div>${val !== null && val !== undefined ? val : ''}</div>`;
          });
          rowsHtml += `
